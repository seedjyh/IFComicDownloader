...
function PicturePageAnalyse(picture_page_url, pagestr, extra_info)
    ...
    local file_url_table = GetFileRelativeURLTable(pagestr)
    if type(file_url_table) ~= "table" then
        return nil, "error when call GetFileRelativeURLTable(...)"
    end

    local middle_url = GetFileMiddleURL(pagestr)
    if type(middle_url) ~= "string" then
        return nil, "error when call GetFileMiddleURL(...)"
    end

    local root_url = GetFileRootURL(picture_page_url)
    if type(root_url) ~= "string" then
        return nil, "error when call GetFileRootURL(...)"
    end

    local file_url = root_url .. middle_url .. file_url_table[picture_index_num]
...
function GetFileRelativeURLTable(pagestr)
    local start_index = JumpStr(pagestr, 1, "var sFiles=\"", 1)
    if type(start_index) ~= "number" then
        return nil, "error when call JumpStr var sFiles=\""
    end
    
    local sFiles = GetStr(pagestr, start_index, "\"")
    if type(sFiles) ~= "string" then
        return nil, "error when getting sFiles."
    end
    
    return unsuan(sFiles)
end

function GetFileMiddleURL(pagestr)
    local start_index = JumpStr(pagestr, 1, "var sPath=\"", 1)
    if type(start_index) ~= "number" then
        return nil, "error when call JumpStr var sPath=\""
    end
    
    local sPath = GetStr(pagestr, start_index, "\"")
    if type(sPath) ~= "string" then
        return nil, "error when getting sPath."
    end
    
    return sPath
end

function GetFileRootURL(picture_page_url)
    -- from 'http://99mh.com/script/ds.js'
    return 'http://images.99mh.com/'
end

function GetPageIndexFromURL(url)
	local index = JumpStr(url, 1, "p=", 1)
	if type(index) ~= "number" then
		return nil, "Can\'t find p= from url " .. url
	end

	local page_index_str = GetStr(url, index, "&")
