...
function VolumePageAnalyse(volume_page_url, pagestr, extra_info)
    ...
    local start_index = JumpStr(pagestr, 1, "var DM5_CTITLE = \"", 1);
    if type(start_index) ~= "number" then
        return nil, function_name .. "failed when jumping var DM5_CTITLE = \""
    end
    ...
    start_index = JumpStr(pagestr, 1, "var DM5_IMAGE_COUNT=", 1);
    if type(start_index) ~= "number" then
    ...
    local now_start = 1
    local index = 1
    local picture_url_root = GetLongestHost(volume_page_url)
    while index <= picture_count_number do
        local picture_url = picture_url_root .. "#ipg" .. tostring(index)
