...
function PicturePageAnalyse(picture_page_url, pagestr, extra_info)
    ...
    -- getting file URL
    local start_index = JumpStr(pagestr, 1, "var images = ", 1)
    if type(start_index) ~= "number" then
        return nil, "error when looking for eval("
    end
    
    start_index = JumpStr(pagestr, start_index, "\"", picture_index_num * 2 - 1)
    if type(start_index) ~= "number" then
        return nil, "error when looking for file URL"
    end
    local raw_file_url = GetStr(pagestr, start_index, "\"")
    if type(raw_file_url) ~= "string" then
        return nil, "error when trying to get file URL string"
    end
    
    file_url = string.gsub(raw_file_url, "\\", "")
