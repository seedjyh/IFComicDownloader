...
function VolumePageAnalyse(volume_page_url, pagestr, extra_info)
    ...
    local start_index = JumpStr(pagestr, 1, "<span>", 1);
    if type(start_index) ~= "number" then
        return nil, function_name .. "failed when jumping <span>"
    end
    ...
    start_index = JumpStr(pagestr, 1, "var totalPage = ", 1);
    if type(start_index) ~= "number" then
        return nil, function_name .. "failed when jumping var totalPage = "
    end
    ...
    while index <= picture_count_number do
        local picture_url = volume_page_url .. "&p=" .. tostring(index)
