...
function ComicPageAnalyse(comic_page_url, pagestr, extra_info)
    ...
    result = result .. TransUtf8ToAnsi(comic_title)
    ----------------------------------------------------------
    result = result .. "</comictitle>"
    ----------------------------------------------------------------------------
    ...
    while true do
        local page_start_index = JumpStr(pagestr, search_start_index, "chapters", 1)
        if type(page_start_index) ~= "number" then
            break
        end
        page_start_index = JumpStr(pagestr, page_start_index, "clearfix", 1)
        if type(page_start_index) ~= "number" then
            break
        end
        local page_end_index = JumpStr(pagestr, page_start_index, "</div>", 1)
        if type(page_end_index) ~= "number" then
            return nil, "Unknown format: no <div class=\"chapters\">"
        end
