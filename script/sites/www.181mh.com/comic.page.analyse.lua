...
function ComicPageAnalyse(comic_page_url, pagestr, extra_info)
    ...
    ----------------------------------------------------------
    local result_end_index=JumpStr(pagestr, 1, "<h1><span>", 1)
    if type(result_end_index) ~= "number" then
        return nil, "JumpStr <h1><span> failed"
    end
    ...
    -- from first "comic-chapters" to first "comment", one volume for each <li>
    while true do
        local page_start_index = JumpStr(pagestr, search_start_index, "comic-chapters", 1)
        if type(page_start_index) ~= "number" then
            break
        end
        local page_end_index = JumpStr(pagestr, page_start_index, "comment", 1)
        if type(page_end_index) ~= "number" then
            return nil, "Unknown format: no comment"
        end
        ...
        while true do
            ...
            if type(index_in_page) ~= "number" then
                return nil, "Unknown format: no title <span> for this volume"
            end
