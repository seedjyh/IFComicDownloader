...
function VolumePageAnalyse(volume_page_url, pagestr, extra_info)
    ...
    if type(start_index) ~= "number" then
        return nil, function_name .. "failed when jumping <h1>"
    end
    ...
    start_index = JumpStr(pagestr, 1, "\"count\":", 2);
    if type(start_index) ~= "number" then
        return nil, function_name .. "failed when jumping \"count\":"
    end
    ...
    local picture_count_number = tonumber(picture_count_str) + 1
