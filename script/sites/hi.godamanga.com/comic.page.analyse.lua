...
function ComicPageAnalyse(comic_page_url, pagestr, extra_info)
    ...
    ----------------------------------------------------------
    local result_end_index=JumpStr(pagestr, 1, "\"name\": \"", 1)
    if type(result_end_index) ~= "number" then
        return nil, "JumpStr \"name\": \" failed"
    end
    ...
    chapter_list_url_pos=JumpStr(pagestr, 1, "class=\"chapter_list_title\"", 1)
    if type(chapter_list_url_pos) ~= "number" then
        return nil, "JumpStr class=\"chapter_list_title\" failed"
    end
    chapter_list_url_pos=JumpStr(pagestr, chapter_list_url_pos, "<a href=\"", 1)
    if type(chapter_list_url_pos) ~= "number" then
        return nil, "JumpStr <a href=\" failed"
    end
    local chapter_list_url=GetStr(pagestr, chapter_list_url_pos, "\"", 1)
    if type(chapter_list_url) ~= "string" then
        return nil, "Unknown format: no \" for <a href=\""
    end
    local pagestr, err = DownloadURL(chapter_list_url, comic_page_url, "")
    if type(pagestr) ~= "string" then
        return nil, "DownloadURL " .. chapter_list_url .. " failed\nbecasuse " .. err
    end
    ...
    while true do
        local page_start_index = JumpStr(pagestr, search_start_index, "<ul class=\"main version-chaps\">", 1)
        ...
        if type(page_end_index) ~= "number" then
            return nil, "Unknown format: no </ul> for <ul class=\"main version-chaps\">"
        end
        ...
        while true do
            ...
            result = result .. "<volumeurl>" .. volume_relative_url .. "</volumeurl>"
            result = result .. "</volumeinfo>"
        end
