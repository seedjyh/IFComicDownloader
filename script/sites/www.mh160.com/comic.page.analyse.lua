...
function ComicPageAnalyse(comic_page_url, pagestr, extra_info)
    ...
    for _, v in pairs(volume_info_list) do
        ...
        result = result .. "<volumeurl>" .. v.url .. "?mode=1</volumeurl>"
        result = result .. "</volumeinfo>"
    end
    result = result .. "</volumeinfolist>"
    ----------------------------------------------------------------------------
...
function ComicPageAnalyse_GetVolumeInfoList(url, pagestr)
    local start_index = JumpStr(pagestr, 1, "plist pnormal", 1)
    if not start_index then return nil, "ComicPageAnalyse_GetVolumeInfoList failed because no plist pnormal" end
    ...
    while true do
        ...
        if not start_index then return nil, "ComicPageAnalyse_GetVolumeInfoList failed because no >" end
        new_info.title = GetStr(pagestr, start_index, "<")
